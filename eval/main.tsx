...
type Ideal = { file: string; source: string; reviewed: boolean; notes?: string; review?: { verdict: string; evidence: string; model: string } };
type Case = {
...
/** One symbol drawn in its exemplar's style, with the engine's catalog drawing of the same thing. */
type SymbolEntry = {
  id: string; label: string; url: string; width: number; height: number;
  engine: string | null; engineSvg: string | null; engineUrl: string | null;
  dsl: string[]; standard: string; sourceUrl: string | null; inExemplar: boolean; notes: string;
  tier: 1 | 2 | 3 | null; usageUsers: number | null;
};
type SymbolSet = {
  type: string; variant: string | null; exemplar: string; style: string; symbols: SymbolEntry[]; problems: string[];
  /** inventory.md: every symbol the type needs, by tier. */
  inventory: string | null;
};
type SymbolsFile = { generatedAt: string; sets: SymbolSet[] };

type Variant = { id: string; label: string; about: string; default?: boolean };
...
function issueOf(s: Slot): Issue | null {
  ...
  if (s.reviewed < s.cases.length) {
    const missing = s.cases.filter((c) => !c.hasIdeal).length;
    const rejected = s.cases.filter((c) => !isReviewed(c) && c.ideal?.review?.verdict === "needs-change").length;
    const pending = s.cases.length - s.reviewed - missing - rejected;
    const parts = [missing && `${missing} targets to draw`, rejected && `${rejected} targets to revise`, pending && `${pending} targets to review`].filter(Boolean);
    return { rank: 4, kind: "targets", text: parts.join(" · ") };
  }
  return null;
}

const RANK_HEAD: Record<number, string> = { 1: "Needs cases", 2: "Only one case", 3: "Draw the exemplar", 4: "Targets need attention" };
...
/** #/ , #/exemplars , #/symbols[/<type>] , #/<type> , #/<type>/<variant> , #/<type>[/<variant>]/<caseId> */
function useRoute() {
...
/** preview/visual-eval/symbols.json, written by scripts/visual-eval/symbols.mts. A missing
 *  manifest is "no symbols drawn yet", not an error. */
function useSymbols() {
  const [file, setFile] = useState<SymbolsFile | null>(null);
  useEffect(() => {
    fetch(SYMBOLS_URL, { cache: "no-store" })
      .then((r) => (r.ok ? (r.json() as Promise<SymbolsFile>) : null))
      .then(setFile)
      .catch(() => setFile(null));
  }, []);
  return file;
...
function TargetNotice({ c }: { c: Case }) {
  ...
  if (!c.ideal.reviewed && c.ideal.review)
    return <p className="banner">Target {c.ideal.review.verdict === "needs-change" ? "needs revision" : "needs further review"}: {c.ideal.review.evidence} Scores are provisional.</p>;
  if (!c.ideal.reviewed)
...
/** One Markdown block: a list, a pipe table, a heading (with anything under it), or a paragraph. */
function NoteBlock({ lines }: { lines: string[] }) {
  if (/^#+ /.test(lines[0]!)) {
    const rest = lines.slice(1);
    return <>
      <h3>{lines[0]!.replace(/^#+ /, "")}</h3>
      {rest.length ? <NoteBlock lines={rest} /> : null}
    </>;
  }
  if (lines.every((l) => /^\s*[-*] /.test(l)))
    return <ul>{lines.map((l, j) => <li key={j}>{inline(l.replace(/^\s*[-*] /, ""))}</li>)}</ul>;
  if (lines.every((l) => /^\s*\|.*\|\s*$/.test(l))) {
    const rows = lines.filter((l) => !/^\s*\|[\s:|-]+\|\s*$/.test(l)).map((l) => l.trim().slice(1, -1).split("|").map((c) => c.trim()));
...
function Notes({ text }: { text: string }) {
  ...
  return (
    <div className="notes">
      {blocks.map((block, i) => <NoteBlock key={i} lines={block.split("\n")} />)}
    </div>
  );
}
...
type Demand = {
  users: number; diagrams: number; genUsers: number; paidUsers: number; newUsers: number; converted: number;
  rubric: { obligation: number; substitute: number; stakes: number; standard: number };
  why: string;
  /** The type's most-searched tool keyword ("… maker", "… software", …) and its bare name, per month. */
  search: { keyword: string; volume: number; cpc: number | null; head: string; headVolume: number; note?: string };
};
type DemandFile = {
  window: string;
  baseline: { genUsers: number; paidUsers: number; newUsers: number; converted: number };
  search: { source: string; pulled: string };
  types: Record<string, Demand>;
};
// demand.json is maintained by hand in exactly this shape (see docs/system/VISUAL-EVAL.md).
const DEMAND = demandData as DemandFile;
type Order = "cluster" | "wtp" | "usage" | "search";
const ORDERS: [Order, string][] = [
  ["cluster", "By cluster"], ["wtp", "By willingness to pay"], ["usage", "By usage"], ["search", "By search volume"],
];

/** People who got a diagram of this type, as a level 0–3: 3 for 1,000+, 2 for 300+, 1 for 100+. */
const usageLevel = (d: Demand) => (d.users >= 1000 ? 3 : d.users >= 300 ? 2 : d.users >= 100 ? 1 : 0);

/** How often this type's users pay, relative to an average ChatDiagram user (1 = average).
 *  Averages two measures, each pulled toward the site-wide rate so a type with few users reads as average. */
function paidSignal(d: Demand) {
  const b = DEMAND.baseline;
  const paidRate = b.paidUsers / b.genUsers, convRate = b.converted / b.newUsers;
  const paid = (d.paidUsers + 200 * paidRate) / (d.genUsers + 200) / paidRate;
  const conv = (d.converted + 500 * convRate) / (d.newUsers + 500) / convRate;
  return (paid + conv) / 2;
}

/** The judgement sets the level; payment data with 150+ users moves it one step when clearly off average. */
function wtpOf(d: Demand) {
  const r = d.rubric;
  const total = r.obligation + r.substitute + r.stakes + r.standard;
  const base = total >= 7 ? 3 : total >= 5 ? 2 : total >= 3 ? 1 : 0;
  const signal = paidSignal(d);
  const enough = d.genUsers >= 150;
  const nudge = enough && signal >= 1.3 ? 1 : enough && signal <= 0.75 ? -1 : 0;
  const level = Math.min(3, Math.max(0, base + nudge));
  return { level, total, signal, shift: level - base, enough };
}

const wtpRank = (type: string): [number, number, number] => {
  const d = DEMAND.types[type];
  if (!d) return [9, 0, 0];
  const w = wtpOf(d);
  return [-w.level, -w.total, -w.signal];
};
const byWtp = (a: { type: string }, b: { type: string }) => {
  const x = wtpRank(a.type), y = wtpRank(b.type);
  return x[0] - y[0] || x[1] - y[1] || x[2] - y[2];
};
const byUsage = (a: { type: string }, b: { type: string }) =>
  (DEMAND.types[b.type]?.users ?? -1) - (DEMAND.types[a.type]?.users ?? -1);

/** Monthly searches for the benchmark tool keyword, as a level 0–3: 3 for 5,000+, 2 for 1,000+, 1 for 200+. */
const searchLevel = (d: Demand) => (d.search.volume >= 5000 ? 3 : d.search.volume >= 1000 ? 2 : d.search.volume >= 200 ? 1 : 0);
const bySearch = (a: { type: string }, b: { type: string }) =>
  (DEMAND.types[b.type]?.search.volume ?? -1) - (DEMAND.types[a.type]?.search.volume ?? -1);

/** Demand levels 1–3, slowest to fastest; level 0 shows a dash. */
const LEVELS = [["🚗", "some"], ["✈️", "a lot"], ["🚀", "the most"]] as const;

function Level({ level, label, detail }: { level: number; label: string; detail: string }) {
  const mark = LEVELS[level - 1];
  return (
    <span className={mark ? "level" : "level level-none"} role="img" tabIndex={0} title={detail}
      aria-label={`${label}: ${level} of 3. ${detail}`}>
      {mark ? mark[0] : "–"}
    </span>
  );
}

function DemandCells({ type }: { type: string }) {
  const d = DEMAND.types[type];
  if (!d) return <><td className="muted">–</td><td className="muted">–</td><td className="muted">–</td></>;
  const w = wtpOf(d), r = d.rubric;
  const count = (n: number) => n.toLocaleString("en-US");
  const usage = `${count(d.users)} people got a ${type} diagram on ChatDiagram in ${DEMAND.window} (${count(d.diagrams)} diagrams).`;
  const moved = w.shift > 0 ? "One level up: its users pay clearly more often than average."
    : w.shift < 0 ? "One level down: its users pay clearly less often than average." : "";
  const wtp = [
    d.why,
    `Judgement ${w.total}/8: professional obligation ${r.obligation}, no good free substitute ${r.substitute}, cost of a wrong drawing ${r.stakes}, checkable standard ${r.standard}.`,
    `On ChatDiagram its users pay ${w.signal.toFixed(1)}× as often as average (${d.paidUsers} of ${d.genUsers} on a paid plan; ${d.converted} of ${d.newUsers} new users subscribed within 30 days).`,
    w.enough ? moved : "Too few users for the payment data to change the level.",
  ].filter(Boolean).join(" ");
  const sv = d.search;
  const search = [
    `"${sv.keyword}" is searched ${count(sv.volume)} times a month (${DEMAND.search.source}, ${DEMAND.search.pulled})`
      + (sv.cpc ? `, and advertisers bid about $${sv.cpc.toFixed(2)} a click.` : "."),
    `The bare name "${sv.head}" gets ${count(sv.headVolume)}, mostly people looking for a definition or an example.`,
    sv.note ?? "",
  ].filter(Boolean).join(" ");
  return <>
    <td className="demand-cell">
      <Level level={usageLevel(d)} label="Usage" detail={usage} />
      <span className="demand-n mono">{count(d.users)}</span>
    </td>
    <td className="demand-cell">
      <Level level={w.level} label="Willingness to pay" detail={wtp} />
      {w.shift ? <span className="demand-n" title={moved}>{w.shift > 0 ? "↑ pays more" : "↓ pays less"}</span> : null}
    </td>
    <td className="demand-cell">
      <Level level={searchLevel(d)} label="Search volume" detail={search} />
      <span className="demand-n mono">{count(sv.volume)}</span>
      <span className="demand-kw" title={sv.keyword}>{sv.keyword}</span>
    </td>
  </>;
}

/** Makes a wheel over the board's scroll box move the page too, so the two feel like one scroll.
 *  Going down, the page moves until the box reaches the top bar, then the table, then the page again;
 *  going up, the table returns to its top before the page moves. Sideways wheels and touch stay native. */
function passWheelToPage(box: HTMLElement) {
  const onWheel = (e: WheelEvent) => {
    ...
    let pageStep: number, tableStep: number;
    if (dy > 0) {
      const pageRoom = page.scrollHeight - window.innerHeight - window.scrollY;
      const barBottom = document.querySelector(".top")?.getBoundingClientRect().bottom ?? 0;
      const untilPinned = Math.max(0, box.getBoundingClientRect().top - barBottom - 8);
      tableStep = Math.min(dy - Math.min(dy, untilPinned, pageRoom), box.scrollHeight - box.clientHeight - box.scrollTop);
      pageStep = Math.min(pageRoom, dy - tableStep);
    } else {
      tableStep = Math.max(dy, -box.scrollTop);
      pageStep = Math.max(dy - tableStep, -window.scrollY);
    }
    if (pageStep === 0 && tableStep === 0) return;
    e.preventDefault();
    box.scrollTop += tableStep;
...
function Board({ coverage, cases, query, left, right, versionLabel }: {
  ...
  const [order, setOrder] = useState<Order>("cluster");
  ...
  const groups = (order === "cluster"
    ? CLUSTER_ORDER.map((cluster) => ({ cluster, types: rows.filter((t) => t.cluster === cluster) }))
    : [{ cluster: "", types: [...rows].sort(order === "wtp" ? byWtp : order === "search" ? bySearch : byUsage) }])
    .filter((g) => g.types.length);
  const boardBox = useRef<HTMLDivElement>(null);
  const hasRows = groups.length > 0;
  useEffect(() => (hasRows && boardBox.current ? passWheelToPage(boardBox.current) : undefined), [hasRows]);
  const judged = cases.some((c) => c.versions.some((v) => v.key === left && v.judged));
  const withRight = judged && right !== NONE;
  const columns = 8 + (judged ? 1 : 0) + (withRight ? 2 : 0);
  ...
  return (
    <>
      <div className="board-head">
        ...
        <div className="level-key" aria-label="How to read the demand columns">
          <p className="level-key-marks">
            {LEVELS.map(([mark, word]) => <span key={word}><span aria-hidden="true">{mark}</span> {word}</span>)}
          </p>
          <p>↑ pays more / ↓ pays less: ChatDiagram users of the type pay clearly more or less often than average, so willingness to pay moved one level</p>
          <p className="level-key-hint">Hover a column header for its thresholds, a cell for its numbers</p>
        </div>
      </div>
      <dl className="stats">
      ...
      <div className="chips" role="group" aria-label="Order">
        {ORDERS.map(([id, label]) => (
          <button key={id} type="button" aria-pressed={order === id} onClick={() => setOrder(id)}>{label}</button>
        ))}
      </div>
      {groups.length ? (
        <div className="scroll board-scroll" ref={boardBox}>
          <table className="board">
            <thead>
              <tr>
                ...
                <th scope="col" title={`People who got a diagram of the type on ChatDiagram in ${DEMAND.window}. 🚀 1,000+  ✈️ 300+  🚗 100+`}>Usage</th>
                <th scope="col" title="A judgement on four questions, 0–2 each: professional obligation, no good free substitute, cost of a wrong drawing, checkable standard. 🚀 7–8  ✈️ 5–6  🚗 3–4. Payment data from 150+ users moves it one step when clearly above or below average.">Willingness to pay</th>
                <th scope="col" title="Monthly US searches for the type's most-searched tool keyword (Google Keyword Planner). 🚀 5,000+  ✈️ 1,000+  🚗 200+">Search volume</th>
              </tr>
            </thead>
            {groups.map((g) => {
...
type SymbolReview = { status: "accept" | "change"; note: string };
const REVIEW_KEY = "visual-eval:symbol-review:v1";
const SYMBOL_ZOOMS = [1, 2, 4] as const;
const symbolKey = (set: SymbolSet, s: SymbolEntry) => `${set.type}/${s.id}`;
const svgData = (svg: string) => `data:image/svg+xml;charset=utf-8,${encodeURIComponent(svg)}`;

/** Review marks live in this browser only; the copied summary is how they reach anyone else. */
function loadSymbolReviews(): Record<string, SymbolReview> {
  try {
    // The stored value is only ever written by saveSymbolReviews below.
    return JSON.parse(localStorage.getItem(REVIEW_KEY) ?? "{}") as Record<string, SymbolReview>;
  } catch {
    return {};
  }
}

function reviewSummary(sets: SymbolSet[], reviews: Record<string, SymbolReview>) {
  return sets.map((set) => [
    `${set.type}${set.variant ? ` (${set.variant})` : ""}`,
    ...set.symbols.map((s) => {
      const r = reviews[symbolKey(set, s)];
      if (!r) return `  · ${s.id} — not reviewed`;
      if (r.status === "accept") return `  ✓ ${s.id} — accepted`;
      return `  ✗ ${s.id} — needs change${r.note.trim() ? `: ${r.note.trim()}` : ""}`;
    }),
  ].join("\n")).join("\n\n");
}

function SymbolRow({ s, zoom, grid, review, onReview }: {
  set: SymbolSet; s: SymbolEntry; zoom: number; grid: boolean;
  review?: SymbolReview; onReview: (next: SymbolReview | null) => void;
}) {
  const engineSrc = s.engineUrl ?? (s.engineSvg ? svgData(s.engineSvg) : null);
  const stage = `symbol-stage${grid ? " with-grid" : ""}`;
  return (
    <li className="symbol-row" id={`symbol-${s.id}`}>
      <div className="symbol-pair">
        <figure>
          <div className={stage}>
            <img src={s.url} width={s.width * zoom} height={s.height * zoom} alt={`${s.label}, drawn in the exemplar's style`} />
          </div>
          <figcaption><span className="panel-kind">Exemplar style</span><code>{s.id}</code></figcaption>
        </figure>
        <figure>
          <div className={stage}>
            {engineSrc
              ? <img src={engineSrc} height={s.height * zoom} alt={`${s.label}, as the engine draws it today`} />
              : <span className="muted small">Not in the engine's symbol catalog</span>}
          </div>
          <figcaption><span className="panel-kind">Engine today</span>{s.engine ? <code>{s.engine}</code> : null}</figcaption>
        </figure>
      </div>
      <div className="symbol-info">
        <h3>{s.label}</h3>
        {s.tier || s.usageUsers !== null ? (
          <p className="symbol-meta">
            {s.tier ? <span className={`symbol-tier tier-${s.tier}`}>{["Must", "Should", "Later"][s.tier - 1]}</span> : null}
            {s.usageUsers !== null ? <span>{s.usageUsers.toLocaleString("en-US")} ChatDiagram users in 90 days</span> : null}
          </p>
        ) : null}
        {s.dsl.length ? <p className="mono">{s.dsl.join("  ·  ")}</p> : null}
        <p className="symbol-standard">
          {s.sourceUrl ? <a href={s.sourceUrl} target="_blank" rel="noopener noreferrer">{s.standard}</a> : s.standard}
        </p>
        <p className="symbol-notes">{s.notes}</p>
        {s.inExemplar ? null : <p className="need-targets">Not in the exemplar, so its look is extrapolated from it.</p>}
        <div className="symbol-review" role="group" aria-label={`Review: ${s.label}`}>
          <button type="button" className="accept" aria-pressed={review?.status === "accept"}
            onClick={() => onReview(review?.status === "accept" ? null : { status: "accept", note: "" })}>Accept</button>
          <button type="button" className="change" aria-pressed={review?.status === "change"}
            onClick={() => onReview(review?.status === "change" ? null : { status: "change", note: review?.note ?? "" })}>Needs change</button>
        </div>
        {review?.status === "change" ? (
          <textarea aria-label={`What should change in ${s.label}`} placeholder="What should change?"
            value={review.note} onChange={(e) => onReview({ status: "change", note: e.target.value })} />
        ) : null}
      </div>
    </li>
  );
...
function SymbolsPage({ file, active }: { file: SymbolsFile | null; active?: string }) {
  const [zoom, setZoom] = useState<number>(1);
  const [grid, setGrid] = useState(false);
  const [reviews, setReviews] = useState<Record<string, SymbolReview>>(loadSymbolReviews);
  const [copied, setCopied] = useState(false);
  const sets = file?.sets ?? [];
  const set = sets.find((x) => x.type === active) ?? sets[0];
  const review = (key: string, next: SymbolReview | null) => setReviews((prev) => {
    const out = next ? { ...prev, [key]: next } : Object.fromEntries(Object.entries(prev).filter(([id]) => id !== key));
    try { localStorage.setItem(REVIEW_KEY, JSON.stringify(out)); } catch { /* storage blocked: keep marks for this visit only */ }
    return out;
  });
  const summary = reviewSummary(sets, reviews);
  const copy = () => {
  ...
  return (
    <>
      ...
      <h1>Symbols</h1>
      <p className="lede">
        Each type's symbols, drawn in the style of its exemplar, beside what the engine draws today. The sheet shows
        a type's symbols together at their true relative size, which is where uneven line weights and proportions show.
      </p>
      {!set ? (
        <div className="exemplar-missing">
          <strong>No symbols drawn yet</strong>
          <span>Draw them into <code>visual-eval/symbols/&lt;type&gt;/</code> with a <code>manifest.json</code>, then run <code>scripts/visual-eval/symbols.mts</code>.</span>
        </div>
      ) : (
        <>
          <nav className="tabs" aria-label="Symbol sets">
            {sets.map((x) => (
              <a key={x.type} href={`#/symbols/${x.type}`} aria-current={x === set ? "page" : undefined}>
                {x.type}{x.variant ? ` · ${x.variant}` : ""}
                <span className="count">{x.symbols.filter((s) => reviews[symbolKey(x, s)]).length}/{x.symbols.length}</span>
              </a>
            ))}
          </nav>
          <section className="symbol-set">
            <div className="symbol-head">
              <a className="symbol-exemplar" href={`#/${set.type}${set.variant ? `/${set.variant}` : ""}`}>
                <img src={`${ASSETS}/exemplars/${set.exemplar}/ideal.png`} alt={`The ${set.exemplar} exemplar these symbols copy`} loading="lazy" />
                <span>Style source: the {set.exemplar} exemplar</span>
              </a>
              <div>
                <h2 className="section">Style</h2>
                <p className="symbol-style">{set.style}</p>
                {set.problems.length ? <ul className="symbol-problems">{set.problems.map((p) => <li key={p}>{p}</li>)}</ul> : null}
                <div className="symbol-controls">
                  ...
                  <label className="symbol-grid-toggle">
                    <input type="checkbox" checked={grid} onChange={(e) => setGrid(e.target.checked)} /> Grid
                  </label>
                </div>
              </div>
            </div>
            <h2 className="section">Sheet</h2>
            <div className={`symbol-sheet${grid ? " with-grid" : ""}`}>
              {set.symbols.map((s) => (
                <a key={s.id} href={`#/symbols/${set.type}`} onClick={(e) => { e.preventDefault(); document.getElementById(`symbol-${s.id}`)?.scrollIntoView({ block: "start" }); }}>
                  <img src={s.url} width={s.width * zoom} height={s.height * zoom} alt="" />
                  <span>{s.label}</span>
                </a>
              ))}
            </div>
            <h2 className="section">One by one</h2>
            <ol className="symbol-list">
              {set.symbols.map((s) => (
                <SymbolRow key={s.id} set={set} s={s} zoom={zoom} grid={grid}
                  review={reviews[symbolKey(set, s)]} onReview={(next) => review(symbolKey(set, s), next)} />
              ))}
            </ol>
            {set.inventory ? (
              <details className="symbol-inventory">
                <summary>Inventory — every symbol {set.type} needs, by tier</summary>
                <Notes text={set.inventory} />
              </details>
            ) : null}
          </section>
          <section className="symbol-summary">
            <h2 className="section">Your review</h2>
            <p className="muted small">Marks are kept in this browser. Copy them into the chat to send them on.</p>
            <p><button type="button" className="primary" onClick={copy}>{copied ? "Copied" : "Copy review summary"}</button></p>
            <pre>{summary}</pre>
          </section>
        </>
      )}
  ...
  return (
    <>
      ...
      <p className="lede">
        ...
        {declared ? ` Drawn ${declared.length} ways; each variant has its own exemplar and cases.` : ""}
      </p>
      {declared ? (
        <nav className="tabs" aria-label={`${type} variants`}>
          <a href={`#/${type}${declared.some(v => v.default) ? "/all" : ""}`} aria-current={!variant ? "page" : undefined}>All<span className="count">{total}</span></a>
          {variants.map((s) => (
...
function Rail({ coverage, routeType, onBoard, onGallery, onSymbols, caseCount, exemplarCount, symbolCount }: {
  coverage: TypeCoverage[]; routeType?: string; onBoard: boolean; onGallery: boolean; onSymbols: boolean;
  caseCount: number; exemplarCount: number; symbolCount: number;
}) {
  const groups = CLUSTER_ORDER
  ...
  return (
    <aside className="rail">
      <nav className="rail-top">
        ...
        <a href="#/symbols" className={onSymbols ? "current" : undefined}>Symbols<span className="count">{symbolCount}</span></a>
      </nav>
      {groups.map((g) => (
...
function App() {
  ...
  const onSymbols = head === "symbols";
  const routeType = head && !onGallery && !onSymbols ? head : undefined;
  const typeVariants = routeType ? VARIANTS[routeType] : undefined;
  const routeVariant = typeVariants?.some(v => v.id === second) ? second
    : !second ? typeVariants?.find(v => v.default)?.id : undefined;
  const routeCase = routeType ? (routeVariant ? third : second === "all" ? undefined : second) : undefined;
  ...
  return (
    <>
      ...
      <div className="layout">
        <Rail coverage={coverage} routeType={routeType} onBoard={!head} onGallery={onGallery} onSymbols={onSymbols}
          caseCount={report.cases.length} exemplarCount={flat.length}
          symbolCount={(symbolFile?.sets ?? []).reduce((n, x) => n + x.symbols.length, 0)} />
        <main>
          {onSymbols ? (
            <SymbolsPage file={symbolFile} active={second} />
          ) : onGallery ? (
