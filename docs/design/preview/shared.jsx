// shared.jsx — Shared data, diagram Stub SVGs, TopNav, syntax helpers
// All pages depend on this file. Load it first in index.html.
// =====================================================================
...
// 13-entry diagram registry. Each page drives its UI from this array.
const DIAGRAMS = [
...
// 4 domain clusters. Color positions are stable — theme changes hue, not semantic.
const DOMAINS = {
  relations:  { label: 'Relationships',           blurb: 'Human & biological kinship',      color: 'var(--cat-0)' },
  electrical: { label: 'Electrical & Industrial', blurb: 'Circuits, PLCs, power, timing',   color: 'var(--cat-2)' },
  corporate:  { label: 'Corporate & Legal',       blurb: 'Ownership and entity trees',       color: 'var(--cat-3)' },
  causality:  { label: 'Causality & Analysis',    blurb: 'Cause-and-effect decomposition',   color: 'var(--cat-1)' },
};

// Pre-tokenized DSL samples used by the Hero showcase widget (rotating tabs).
// For the full plain-text editor DSL, see page-playground.jsx → DSL_TEXT.
const DSL_SAMPLES = {
  genogram: [
        ...
        <nav className="primary">
          {links.map(l => (
            <a key={l.id}
               ...
               onClick={() => setRoute(l.id)}>{l.name}</a>
          ))}
          <a>docs</a>
        </nav>
        <div className="right">
...
// ================================================================
// DIAGRAM STUB SVGs — one per diagram type
// All use currentColor so they inherit accent/text from the theme.
// These are miniature faithful renditions of each standard.
// ================================================================
...
function StubEntity() {
  return (
    <svg viewBox="0 0 320 220" style={{width:'100%',height:'100%'}} fill="none" stroke="currentColor" strokeWidth="1.5">
      ...
      <text x="134" y="46" fontFamily="var(--mono)" fontSize="7" fill="var(--text-muted)" stroke="none">SD · trust</text>
      ...
      <text x="130" y="122" fontFamily="var(--mono)" fontSize="7" fill="var(--text-muted)" stroke="none">DE · corp</text>
      ...
      <text x="32" y="202" fontFamily="var(--mono)" fontSize="7" fill="var(--text-muted)" stroke="none">UK · ltd</text>
      ...
      <text x="232" y="202" fontFamily="var(--mono)" fontSize="7" fill="var(--text-muted)" stroke="none">KY · fund</text>
    </svg>
  );
}
...
function StubPhylo() {
  return (
    <svg viewBox="0 0 320 220" style={{width:'100%',height:'100%'}} fill="none" stroke="currentColor" strokeWidth="1.5">
      ...
      <circle cx="80" cy="110" r="8" fill="var(--accent-soft)" stroke="none"/>
      <text x="72" y="113" fontFamily="var(--mono)" fontSize="7" fill="var(--accent-ink)" stroke="none">98</text>
    </svg>
  );
}
...
function StubSociogram() {
  return (
    <svg viewBox="0 0 320 220" style={{width:'100%',height:'100%'}} fill="none" stroke="currentColor" strokeWidth="1.4">
      <g stroke="var(--accent)">
...
function StubBlock() {
  return (
    <svg viewBox="0 0 320 220" style={{width:'100%',height:'100%'}} fill="none" stroke="currentColor" strokeWidth="1.5">
      ...
      <line x1="270" y1="110" x2="305" y2="110"/>
      <polygon points="305,110 298,106 298,114" fill="currentColor" stroke="none"/>
  ...
  return (
    <svg viewBox="0 0 320 220" style={{width:'100%',height:'100%'}} fill="none" stroke="currentColor" strokeWidth="1.5">
      ...
      <g stroke="none" fontFamily="var(--mono)" fontSize="8" fill="var(--text-muted)">
        ...
        <text x="22" y="196">Measurement</text>
...
// Patch Stub refs in DIAGRAMS now that all functions are defined
DIAGRAMS.forEach(d => {
  const fnMap = {
    genogram: StubGenogram, ladder: StubLadder, entity: StubEntity,
    pedigree: StubPedigree, phylo: StubPhylo, sld: StubSLD,
    circuit: StubCircuit, logic: StubLogic, timing: StubTiming,
    ecomap: StubEcomap, sociogram: StubSociogram, block: StubBlock,
    fishbone: StubFishbone,
  };
  d.Stub = fnMap[d.id];
});
...
Object.assign(window, {
  ...
  tokenToClass, renderTokens, TopNav,
