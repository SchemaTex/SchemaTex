...
> **Status:** IMPLEMENTED. DFMEA/PFMEA Action Priority uses the complete published rating-band matrix, verified against [HBK ReliaSoft’s primary implementation reference](https://reliasoftcloud.hbkworld.com/demo/help/content/risk_ranking_logic.htm) on 2026-09-14. FMEA-MSR uses different inputs and remains unsupported.
...
2. **Action Priority** per row: resolve `(S, O, D)` with the DFMEA/PFMEA band matrix in the reference above. Severity is a primary dimension, but **S = 9–10 does not automatically imply High**: S10/O2/D3 is Low, S9/O2/D5 is Medium, and S9/O2/D7 is High. The implementation covers the complete integer domain 1–10; it does not approximate AP from RPN. This is not the separate FMEA-MSR S/F/M mapping.
