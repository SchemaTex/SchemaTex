...
| **`commit id: "x"`** | explicit commit id (else auto `n-hash`) | id printed beside the dot as **horizontal text**; `rotateCommitLabel: true` tilts it ~45° |
...
- **Tags** render as a small pill/flag at the commit; **commit ids** render beside the dot as **horizontal text**, with auto-ids shaped `n-<hash>`. `rotateCommitLabel: true` tilts them ~45°.
