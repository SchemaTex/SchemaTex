...
| 模式 | 说明 | 适用场景 |
|------|------|---------|
| `swimlane` | **默认** | 多轨道（传记、产品路线图）；range bars 在轨道中；point/milestone 标记在轴上方 |
| `gantt` | 项目计划风格 | 里程碑 pin 在轴上方，任务 bar 按 `category` 分组在轴下方，右侧图例 |
| `lollipop` | 棒棒糖图 | 中心轴，事件卡片交替出现在轴上下两侧，茎连接轴 |

DSL：`config: style = swimlane | gantt | lollipop`

#### Gantt 专用属性

- `[category: "Design"]` — 将 range event 归入具名泳道；对应图例颜色自动生成
- point/milestone 自动升至 pin zone（轴上方），range 自动进入对应 category lane

#### Lollipop 专用属性

- `[side: above]` / `[side: below]` — 强制卡片位置；默认交替（above → below → above…）
- 轴端各留 card 半宽 + 8px 的 inset，防止首尾卡片悬挂到图外
- Era 背景带在 lollipop 模式下自动延伸到 `plotX` / `plotRight`，确保覆盖边缘卡片

### 4.4 Scale mapping 公式
...
config_key      = "orientation" | "scale" | "style" | "axis" | "dateFormat"
...
property        = "color:" HEX
                ...
                | "category:" quoted_string                 # gantt lane grouping
                | "side:" ( "above" | "below" )             # lollipop card placement
                | "data:" quoted_string
...
### TC-TL-01 · Single event (smoke test)
...
**验证：** 单事件不崩溃，canvas 合理，axis 显示单 tick。

---

### TC-TL-02 · Basic point events — proportional scale
...
1969-07-20: milestone "Apollo 11 Moon landing" [icon:🚀]
1971-04-19: "Salyut 1 first space station"
1981-04-12: "STS-1 Space Shuttle"
1998-11-20: "ISS first module"
```

**验证：** 按比例分布，labels 双向级联避让无重叠，milestone ★ 渲染。

---

### TC-TL-03 · Range (lifespan) + milestone

```
timeline "Einstein — key moments"

1879 - 1955: "Albert Einstein"
1905: milestone "Annus Mirabilis"
1915: milestone "General Relativity"
1921: milestone "Nobel Prize" [icon:🏆]
```

**验证：** Lifespan bar 覆盖正确年份，milestone 叠加在对应 x 坐标。

---

### TC-TL-04 · Era bands + milestones

```
timeline "WWII European Front"

era 1939-09 - 1940-05: "Phoney War"
...
1941-06-22: milestone "Barbarossa"
1942-08 - 1943-02: "Stalingrad"
1944-06-06: milestone "D-Day"
1945-05-08: milestone "VE Day"
```

**验证：** 5 个连续 era 背景带，milestones 在关键节点，range events 精确。

---

### TC-TL-05 · Named tracks (Einstein multi-track)
...
track "Career":
  ...
  1902 - 1909: "Swiss Patent Office"
  1905: milestone "Annus Mirabilis"
    note: "4 breakthrough papers."
  1914 - 1933: "Berlin / Kaiser Wilhelm"
  1915: milestone "General Relativity"
  1921: milestone "Nobel Prize" [icon:🏆]
  1933 - 1955: "Princeton IAS"
...
track "Public":
  1939: "Einstein–Szilárd letter" [icon:✉️]
  1952: "Declined Israel presidency"
  1955-04-18: "Died in Princeton" [icon:🕯]
```

**验证：** 3 swimlanes，eras 背景，note callout，icon 渲染。

---

### TC-TL-06 · Product roadmap (equidistant, 3 swimlanes)
...
track "Community":
  ...
  2026-Q3: milestone "1000 stars + first external PR"
```

**验证：** Q1–Q4 均分，milestones ★，3 swimlanes 整齐。

---

### TC-TL-07 · BC/AD historical eras (Europe 500 BC – 1700 AD)
...
-753: milestone "Founding of Rome"
...
**验证：** 负数年份正确解析，era 重叠时 band row 堆叠，BC/AD 跨越 0 年无偏移。

---

### TC-TL-08 · Covid proportional + auto-pack
...
**验证：** 密集 2020 段事件 label cascade 不重叠，2023 独立显示。

---

### TC-TL-09 · Geological timescale (log, Ma)
...
4000Ma: "Oldest rocks"
3500Ma: "First life"
2400Ma: "Great Oxidation"
540Ma: milestone "Cambrian Explosion"
252Ma: milestone "Permian-Triassic extinction"
66Ma: milestone "K-Pg extinction" [icon:☄️]
0.3Ma: "Homo sapiens emerges"
0.012Ma: "Holocene begins"
```

**验证：** log 轴 3 个量级跨度分布合理，Holocene 可见。

---

### TC-TL-10 · Apollo program (ranges + auto-pack)
...
**验证：** ranges + points 混合，auto-pack，short-duration bars 可见。

---

### TC-TL-11 · Icon + shape + color overrides
...
2021: milestone "Perseverance on Mars" [color:#6a1b9a, icon:🔴]
```
...
### TC-TL-12 · Company history (ranges + milestones)
...
2020 - 2024: "Remote-first scaling"
2021: milestone "Series A $15M"
2023: milestone "Series B $50M"
2025: milestone "IPO" [icon:🎉]
```

---

### TC-TL-13 · Gantt style — project plan
...
2026-04-15 - 2026-05-25: "UI mockups" [category: "Design"]
2026-04-10 - 2026-06-10: "API build" [category: "Eng"]
2026-05-01 - 2026-06-20: "Dashboard" [category: "Eng"]
2026-05-15 - 2026-06-25: "Campaign prep" [category: "Marketing"]
2026-06-10 - 2026-06-30: "Rollout" [category: "Ops"]
```

**验证：** Milestone pins 在轴上方，task bars 按 category 分组，右侧图例，颜色对应。

---

### TC-TL-14 · Gantt style — overlapping tasks per team

```
timeline "Engineering roadmap H1"
config: style = gantt

2026-01-15: milestone "Planning done"
2026-03-01: "Alpha"
2026-05-01: milestone "Beta" [icon:🎯]
2026-06-15: milestone "GA" [icon:🚀]

2026-01-01 - 2026-02-28: "Architecture" [category: "Platform"]
2026-02-01 - 2026-04-30: "Auth service" [category: "Platform"]
2026-02-15 - 2026-05-30: "Billing" [category: "Platform"]
2026-01-15 - 2026-03-30: "Onboarding UX" [category: "Product"]
2026-03-15 - 2026-06-15: "Dashboard v2" [category: "Product"]
2026-04-01 - 2026-06-15: "Mobile app" [category: "Mobile"]
2026-05-01 - 2026-06-15: "Launch campaign" [category: "Growth"]
```

**验证：** 同 category 内 bars 多行 pack，pin label cascade，4 个图例项。

---

### TC-TL-15 · Lollipop — auto-alternate
...
1996: "Founded" [icon:🎓]
2001: milestone "First patent" [icon:💡]
2005: "Product launch" [icon:🚀]
2012: "Global expansion" [icon:🌍]
2018: milestone "IPO" [icon:🏆]
2024: "Hit $1B revenue" [icon:📈]
```

**验证：** 6 张卡片自动上下交替，stems 等高，all same-side cards 水平对齐。

---

### TC-TL-16 · Lollipop — explicit side + equidistant
...
1961: "Vostok 1" [icon:🧑‍🚀, side: above]
1969: milestone "Apollo 11" [icon:🌕, side: below]
1971: "Salyut 1" [icon:🛰, side: above]
1981: "Space Shuttle" [icon:🛩, side: below]
1990: "Hubble launch" [icon:🔭, side: above]
1998: "ISS begins" [icon:🏗, side: below]
2012: "Curiosity" [icon:🛞, side: above]
2021: milestone "James Webb" [icon:✨, side: below]
```

**验证：** above 卡片全部水平对齐（同一 stem 高度），below 同理，不因碰撞误升层。

---

### TC-TL-17 · Lollipop — eras background
...
**验证：** Era 背景带延伸到 `plotX` / `plotRight`，首尾卡片完整覆盖；轴基于事件范围（2015–2025），右侧无多余空白。

---

## 12. Implementation Status — **Implemented ✓**

| Phase | 状态 | 内容 |
|-------|------|------|
| 19.0 Parser | ✓ | DSL：config / era / track / event（point/range/milestone）/ note / properties；ISO / year / BC / Ma 日期；Q1–Q4 季度格式 |
| 19.1 Layout (swimlane) | ✓ | proportional / equidistant / log scale；tick granularity 自动选择；greedy interval track packing；label 双向级联碰撞避让 + leader line |
| 19.2 Layout (gantt) | ✓ | milestone pin zone；category lane auto-sort；pin label cascade；legend 数据生成 |
| 19.3 Layout (lollipop) | ✓ | 事件范围轴（不含 era 端点）；card-half inset 防溢出；同侧卡片 2px 容差对齐；leftmost/rightmost era 延伸到 plotX/plotRight |
| 19.4 Renderer | ✓ | 3 种 style dispatch；axis + ticks + eras + ranges + milestones + pins + gantt bars + lollipop cards + legend；theme tokens 集成 |
| 19.5 Test coverage | ✓ | TC-TL-01 ~ TC-TL-17，见 `preview/timeline.html` |
