...
## 2b. Flexibility Options（v2 扩展）

Ishikawa 原始文献固定鱼骨形态，但实际 SaaS 工具（Lucidchart / Miro / Minitab）在模板里对以下维度做了取舍。Schematex 把这些选项显式开放给 DSL 用户，默认值严格遵循 ASQ 模板。

### 2b.1 `sides` — 单侧 vs 双侧

```
config sides = both | top | bottom    # default: both
```

- `both`（默认）— 经典 Ishikawa，上下各半数 ribs
- `top` — 所有 rib 从 spine 向上发散；适合 category 数少、横向空间紧张、或与其它内容上下拼排
- `bottom` — 所有 rib 向下发散；常见于报告正文下方的"成因梳理栏"

当 `sides: top | bottom` 时，任何带 `[side: ...]` 的 category 仍会被强制放到指定半边。

### 2b.2 `[side]` + `[order]` — 每条 rib 手动布局

```
category content "内容"   [side: top, order: 0]
category tech    "技术"   [side: top, order: 1]
category ux      "体验"   [side: bottom, order: 0]
```

- `side: top | bottom` 覆盖自动分配
- `order: <int>` 控制同一半内的左→右顺序（低值靠近鱼尾）
- 未指定 side 的 category 按贪心规则填充剩下的半边

推荐用法：将"关键因素"放 top，"次要因素"放 bottom，或按 RACI/责任方分组。

### 2b.3 `ribSlope` — rib 斜率

```
config slope = gentle | normal | steep    # presets: 0.45 / 0.6 / 0.75
config ribSlope = 0.55                    # 或任意数值 (0, 3)
```

- `gentle` (0.45) — rib 更陡，上下紧凑，适合 label 较长时避免水平拥挤
- `normal` (0.6) — ASQ/Minitab 经验值（默认）
- `steep` (0.75) — rib 更扁，视觉更尖锐；适合 rib 数量少 + 空间充裕

### 2b.4 `density` — 整体密度

```
config density = compact | normal | spacious    # default: normal
```

三档预设同时调节 `rowHeight`、`subRowHeight`、`colStep`、`headerW/H`、`headerGap`、`minRowsPerHalf`、`colFirstOffset`。

| Preset | rowHeight | colStep | headerW × H |
|--------|-----------|---------|-------------|
| compact | 24 | 110 | 118 × 30 |
| normal  | 30 | 130 | 132 × 34 |
| spacious| 36 | 150 | 148 × 38 |

- `compact` — 打印版 / 并列多图 / dashboard 缩略
- `spacious` — 演示幻灯片 / 展板
...
| 场景 | 推荐组合 |
|------|---------|
| 报告正文插图 | `sides: bottom` + `density: compact` |
| 演示幻灯片 | `density: spacious` |
| Category 多 (> 6)、label 长 | `slope: gentle` + `causeSide: both` |
| 单侧紧凑 | `sides: top` + `density: compact` + `slope: gentle` |
| 关键/次要因素对比 | `[side: top]` 放关键 + `[side: bottom]` 放次要 |
