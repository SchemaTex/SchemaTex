...
颜色驱动由 `--schematex-entity-*` CSS custom properties，消费者可完全覆盖。默认 palette：

| CSS Variable | Default | 税务含义 |
|--------------|---------|----------|
| `--schematex-entity-ccorp` | `#dbeafe` | 课税主体（C-Corp） |
| `--schematex-entity-passthrough` | `#dcfce7` | 透过课税（LLC, LP, S-Corp） |
| `--schematex-entity-trust` | `#ede9fe` | 信托 |
| `--schematex-entity-individual` | `#fed7aa` | 自然人 |
| `--schematex-entity-disregarded` | `#f5f5f5` | 税务忽略实体（DRE） |
| `--schematex-entity-foundation` | `#fef9c3` | 非营利 / 基金会 |
| `--schematex-entity-pool` | `#f1f5f9` | 预留池（非独立实体） |
...
继承 Schematex 的 **orthogonal routing** 惯例（与 genogram parent-child edge 相同）：
...
- CSS hover classes: `.schematex-entity-node:hover` 由消费者定制
