...
| Shape keyword | 语义 | Mermaid syntax | SVG 构造 |
|---------------|------|----------------|----------|
| `rect` (default) | 通用 process / task | `A[text]` | `<rect rx=0 x=0 y=0 w=120 h=60/>` |
| `round` | 柔和 process / milestone | `A(text)` | `<rect rx=12 ry=12 w=120 h=60/>` |
| `stadium` | terminator / start-end | `A([text])` | `<rect rx=30 ry=30 w=120 h=60/>` |
| `diamond` | decision / branch | `A{text}` | `<polygon points="60,0 120,30 60,60 0,30"/>` |
| `parallelogram` | input / output data | `A[/text/]` | `<polygon points="20,0 120,0 100,60 0,60"/>` (slant 20px) |

### 4.2 次级形状（M2 — +25 个，共 30 形状）
...
| Feature | M1 | M2 | M3 | Test Case |
|---------|:--:|:--:|:--:|-----------|
| Parse `graph/flowchart` + direction | ✓ | ✓ | ✓ | 1, 5 |
| 5 core shapes | ✓ | ✓ | ✓ | 6 |
| 30 total shapes | — | ✓ | ✓ | 9 |
| 6 core edge types | ✓ | ✓ | ✓ | 2, 10 |
| 14 edge types incl. round/crossed | — | ✓ | ✓ | 10 |
...
| Subgraph flat | — | ✓ | ✓ | 7 |
| Subgraph nested 3+ | — | ✓ | ✓ | 8 |
| Cross-cluster edge | — | ✓ | ✓ | 7, 8 |
| classDef / class / style / linkStyle | — | ✓ | ✓ | 11 |
| Markdown in label | — | — | ✓ | — |
| Animated edges | — | — | ✓ | — |
| Click href | — | — | ✓ | — |
