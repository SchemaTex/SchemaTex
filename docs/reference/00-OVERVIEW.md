...
| 🗓️ **Project management** | PERT / CPM network · IDEF0 function model | PMI PMBOK 7 + Moder 1983 — activity-on-node (AON/PDM) scheduling network; the engine computes ES/EF/LS/LF, total slack, project duration, and the critical path. FS/SS/FF/SF dependencies with lag/lead, three-point estimation (te + variance), milestones, swimlanes, and a time-scaled layout (32-PERT-STANDARD) · FIPS PUB 183 IDEF0 — ICOM arrow placement (Input-left/Control-top/Output-right/Mechanism-bottom), diagonal box staircase, node numbering (45-IDEF0-STANDARD) |
