...
1. *"What is this diagram and who is it for?"* — opening Playground + a short **About {diagram}** section. The section orients a reader who has never heard the term: what the diagram represents, which professionals use it for what, and which published standard Schematex follows. Include 1–3 authoritative external links (the standard's book, a Wikipedia entry, a field-society page) so the page feels grounded, not self-referential. Keep it to ~2 short paragraphs — an orientation, not a literature review. Do NOT also put a prose description in the MDX frontmatter — fumadocs renders it as a lead under the H1 and it duplicates the About section. Frontmatter `title` alone is enough.
