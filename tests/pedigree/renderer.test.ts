...
describe("pedigree renderer", () => {
  ...
  I-1 -- I-2
    II-1 [male]`);
    expect(svg).toContain("schematex-pedigree-gen-label");
  ...
  test("renders presymptomatic line", () => {
    ...
    expect(svg).toContain("schematex-pedigree-presymptomatic-mark");
  });

  test("renders deceased slash", () => {
    const svg = renderFromDSL(`pedigree\n  a [male, deceased]\n  b [female]\n  a -- b`);
    expect(svg).toContain("schematex-pedigree-deceased-mark");
  ...
  I-2 [female]
  I-1 -- I-2`);
    expect(svg).toContain("schematex-pedigree-legend");
