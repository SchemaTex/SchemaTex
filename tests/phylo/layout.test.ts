...
describe("phylo layout — rectangular phylogram", () => {
  test("basic tree produces correct number of nodes", () => {
    const ast = parsePhylo(`phylo [layout: rectangular]\n  newick: "((A:0.1,B:0.2):0.3,C:0.5);"`);
...
describe("phylo layout choice", () => {
  test("default branches meet at each parent; explicit rectangular retains connectors", () => {
    const source = 'phylo\n  newick: "((Oak:0.2,Elm:0.4):0.1,(Pine:0.3,Fir:0.1):0.25);"';
    const slanted = layoutPhylo(parsePhylo(source));
    const rectangular = layoutPhylo(parsePhylo(source.replace("phylo", "phylo [layout: rectangular]")));
    expect(slanted.ast.layout).toBe("slanted");
    expect(slanted.branches.every(b => !b.isConnector)).toBe(true);
    expect(rectangular.branches.some(b => b.isConnector)).toBe(true);
    // A layout preference must not change any distance or topology.
    expect(slanted.nodes.map(n => [n.node.id, n.x])).toEqual(
      rectangular.nodes.map(n => [n.node.id, n.x]));
    for (const branch of slanted.branches) {
      const parent = slanted.nodes.find(n => n.node.id === branch.fromId)!;
      const child = slanted.nodes.find(n => n.node.id === branch.toId)!;
      expect(branch.path).toBe(`M ${parent.x},${parent.y} L ${child.x},${child.y}`);
      expect(child.x - parent.x).toBeCloseTo(child.node.branchLength! * slanted.scale);
    }
  });
});
