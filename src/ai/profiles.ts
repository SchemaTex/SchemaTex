...
const PROFILES: Record<DiagramType, GenerationProfile> = {
  ...
  evacuation: {
    ...
    keywords:
      'evacuation|escapeplan "Title" · compliance iso|nfpa|uae · sheet a3|a4|a2|letter|tabloid [landscape|portrait] · floor N "Label" · room/extend/door/window/opening/furniture/grid/row/arc (same as floorplan) · safety <kind> [id] in <room> at x,y | outside at x,y [side north|south|east|west] [hand left|right] [rotate deg] [class "A|B|C|D|K"] ["label"] · kinds: here exit exit-direction exit-final assembly refuge shelter first-aid aed stretcher doctor eyewash safety-shower emergency-phone break-glass escape-ladder rescue-window emergency-door-push emergency-door-slide extinguisher hose-reel fire-ladder fire-equipment call-point fire-phone riser not-an-exit no-elevator alarm-sounder · common aliases: you-are-here->here emergency-exit/fire-exit->exit exit-arrow/direction-arrow->exit-direction assembly-point/muster-point->assembly fire-extinguisher->extinguisher fire-alarm/alarm-call-point->call-point fire-hose-reel->hose-reel defibrillator->aed · route [primary|secondary|accessible|rescue] <anchor> -> <room-or-symbol> -> <exit-symbol> ["label"] · fire-door|smoke-door <door-ref> [rating "EI30"] · legend on|auto · show furniture',
    forms: [
  ...
  stageplot: {
    ...
    mode:
      "formal live-sound advance + measured stage + positioned equipment + derived input and monitor-output schedules",
    keywords:
      'venue "Text" · show-date "Text" · revision "Text" · technical-contact "Text" · stage id ["Label"] at x,y size WxH · equipment <kind> <id> (in <stage> at x,y | outside at x,y) [size WxH] [rotate deg] ["label"] [channel N] [source "Instrument/person"] [model "Suggested mic or DI"] [stand boom|straight|short-boom|clip|none] [phantom yes|no] [notes "Text"] [mix N] · monitor <mix-number> [id] in <stage> at x,y ["label"] · signal <equipment-id> -> <equipment-id> [-> ...] ["label"] · input-list on|off · output-list on|off · signal-paths on|off · kinds: drum-kit guitar-amp bass-amp keyboard bass-cabinet boom-stand straight-stand drum-mic overhead di-box mixer foh-console snake monitor-wedge side-fill iem power-drop stage-riser music-stand set-list stage dance-floor dj-booth podium row-chairs piano',
    forms: [
    ...
    prefer: [
      ...
      "For a document sent to a venue, add `venue`, `show-date`, `revision`, and `technical-contact`; these populate the formal sheet header.",
      ...
      "Give every wedge, IEM, and side fill a positive `mix`; Schematex derives a monitor-output schedule grouped by console send and endpoint type.",
      "Put `channel`, `source`, `model`, `stand`, `phantom`, and `notes` on each input-bearing mic/DI node. The input list and stage position are auto-derived and sorted by channel.",
      ...
      "Use `signal` for the few operational paths that help the crew, then add `signal-paths on` only when the patch overlay improves the handoff.",
    ],
    avoid: [
      ...
      "Don't leave a wedge, IEM, or side fill unnumbered. Use the `monitor <mix>` shorthand or add `mix N`.",
      "Don't rely on color to explain a channel or monitor send; the renderer prints explicit CH and MIX labels for grayscale output.",
    ...
    repair: [
      ...
      "'needs a positive mix number' -> add `mix N`; wedges may use `monitor N <id> in <stage> at x,y`.",
