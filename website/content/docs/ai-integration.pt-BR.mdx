...
O Schematex inclui oito ferramentas para LLMs que permitem **descobrir**, **gerar**, **validar**, **renderizar**, **inspecionar** e **editar com segurança** diagramas. Três caminhos de integração, os mesmos contratos:

| Caminho | Quando usar | Instalação |
|---|---|---|
| **Vercel AI SDK** | Construindo seu próprio recurso de IA em um app Next.js / Node | `npm i schematex ai` |
...
Todos os caminhos expõem as mesmas oito ferramentas com semântica idêntica.

### `listDiagrams()`
Retorna todos os 51 tipos de diagrama e suas capacidades interativas. Chame primeiro para escolher um tipo.
...
### Edição segura: `getDiagramCapabilities`, `inspectDiagram`, `applyDiagramEdits`

Essas ferramentas retornam o contrato de edição, chaves de alvo estáveis e uma revisão e aplicam mudanças de label ou posição de forma atômica. Uma revisão obsoleta ou edição insegura devolve o DSL original.
...
O subpath `schematex/ai/sdk` precisa apenas do peer-dep opcional `ai`. Ele usa JSON Schema diretamente, portanto Zod não é necessário. Sem o AI SDK, importe as funções de `schematex/ai`:
