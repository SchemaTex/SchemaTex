...
Schematex には、ダイアグラムを**探索**、**生成**、**検証**、**レンダリング**、**調査**、**安全に編集**できる 8 つの LLM ツールが付属します。3 つの統合パスは同じ契約を使います：

| パス | 用途 | インストール |
|---|---|---|
| **Vercel AI SDK** | Next.js / Node アプリに独自の AI 機能を構築する | `npm i schematex ai` |
...
すべてのパスは同じ 8 つのツールを同一のセマンティクスで公開します。

### `listDiagrams()`
51 種類すべてのダイアグラムタイプと Interactive 能力を返します。最初に呼び出してタイプを選択してください。
...
### 安全な編集：`getDiagramCapabilities`、`inspectDiagram`、`applyDiagramEdits`

編集契約、安定した target key、revision を取得し、label または position の変更を atomic に適用します。古い revision や安全でない編集では元の DSL が返ります。
...
`schematex/ai/sdk` サブパスに必要なオプション peer dependency は `ai` だけです。JSON Schema を直接使うため Zod は不要です。AI SDK を使わない場合は `schematex/ai` から関数をインポートしてください：
