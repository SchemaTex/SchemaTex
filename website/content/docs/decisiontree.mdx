...
Questions, answers, and the title edit their exact source ranges. Branch topology and rollback order own the automatic tree layout, so cards intentionally expose no persistent position drag.
