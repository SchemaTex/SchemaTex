...
Wires route around gate bodies and captions. A filled dot marks a branch of the same source signal; a crossing clearance without a dot is not a connection. Symbol interiors follow the selected theme and are opaque. Long input/output labels reserve space automatically. These are drawing guarantees, not electrical simulation or a proof that the author's Boolean function is correct.
