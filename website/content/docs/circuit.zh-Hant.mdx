...
照明負載應明確寫成 `type=lamp`，因為 SPICE 慣例中，裸 `L` 前綴代表**電感**。電源使用清楚的 `live` / `neutral` 網路名；自動佈局會將保護與控制的火線路徑放在上方，零線回路放在下方。

```text
circuit "住宅單控照明" netlist
...
雙控（樓梯燈）必須使用兩個 SPDT 開關並共享兩條 traveler 網路。將多個 SPST 開關簡單串聯會改變電路功能，無法實現正常雙控。

```text
circuit "樓梯雙控照明" netlist
V1 live neutral 220Vac type=acsource label="V_mains"
F1 live feed 16A
...
燈具使用 `L1`（或其他適合負載的標號）；只有元件確實是繼電器或負載電阻時才使用 `RL1`。
