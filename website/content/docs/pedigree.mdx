...
Following NSGC Bennett 2022 §4.5, all carrier statuses use a diagonal hatch fill whose meaning is defined in the key.
...
| `carrier` | Diagonal hatch fill — autosomal carrier |
| `carrier-x` | Diagonal hatch fill — X-linked carrier |
| `obligate-carrier` | Diagonal hatch fill — carrier inferred from pedigree structure |
| `presymptomatic` | Vertical line from edge to edge through the shape — tested positive, no clinical signs yet |
...
| `stillborn` | Sex shape with the deceased diagonal slash + "SB" label |
| `pregnancy` | "P" inside the sex shape (diamond if sex unknown) |
| `sab` | Small triangle + "SAB" label — spontaneous abortion |
| `tab` | Small triangle with a diagonal termination slash — terminated pregnancy |
| `ectopic` | Small triangle with a diagonal termination slash + "ECT" label |
...
`-/-` means the relationship no longer exists; it draws a continuous line with two diagonal slashes.
