...
`LR` (default), `TB`, and `BT` are accepted. Commit labels are horizontal by default (`rotateCommitLabel: false`); enable rotation explicitly when needed. Config can also be supplied via a leading `%%{init: {'gitGraph': {...}}}%%` directive or a YAML frontmatter `config:` block for Mermaid compatibility (`mainBranchName`, `showCommitLabel`, `rotateCommitLabel`).
