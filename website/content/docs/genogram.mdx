...
|---|---|---|
| Sex | `male`, `female`, `unknown`, `other` | Shape: square, circle, question mark, question mark |
...
| `external:true` | `true` or `false` | Dashed outline for an outside support contact. With only emotional ties, placed beside directly connected family members; family relationships take precedence. |
| `key:value` | any custom | Stored as metadata only; does not affect rendering or layout. |

Describe people and relationships; the engine chooses positions and routes.
Household boundaries are not currently supported. A custom `household:` attribute
will not draw a boundary.
...
| ? Question mark | `unknown`, `other`, *or* attribute omitted | Unknown / unspecified; explicit `shape:diamond` still draws a diamond |

Status symbols follow the McGoldrick Multicultural Family Institute (MFI) 2017 handout:

| Status | Drawing |
|---|---|
| `deceased` | X through the sex shape |
| `miscarriage` | Small hollow circle (spontaneous abortion) |
| `abortion` | Small x (induced abortion) |
| `stillborn` | Small square with an X, without an SB label |
...
The McGoldrick MFI 2017 handout supplies the relationship notation it defines; GenoPro supplies the additional relationship marks.

| Type | Drawing |
|---|---|
| `distant` | Short-dash line |
| `distant-hostile` | Zigzag over a dashed line |
| `abuse`, `physical-abuse` | Zigzag with a filled arrowhead |
| `emotional-abuse` | Zigzag with a hollow arrowhead |
| `sexual-abuse` | Zigzag between two parallel lines with a filled arrowhead |
| `love` / `inlove` | One hollow circle / two overlapping hollow circles mid-line |
| `admirer` / `limerence` | Open arrow with one hollow circle / two overlapping hollow circles |
| `distrust` | Row of short perpendicular ticks along the line |
| `manipulative` / `controlling` | Open arrow with an X / boxed X mid-line |
| `jealous` | Open arrow with a hollow diamond mid-line |
| `nevermet` | Boxed X mid-line |
| `neglect` | Dashed line with an open arrowhead |
| `bestfriends` | Two parallel lines with short cross ticks |

Emotional-line colors follow the active theme's `positive`, `negative`, `neutral`, `accent`, and `warn` inks; they are not fixed hex values.
...
Schematex genograms follow **McGoldrick, Gerson & Petry (2020), *Genograms: Assessment and Treatment*, 4th ed.** for family structure and generation alignment, with status symbols and relationship notation from the **McGoldrick MFI 2017 handout**. Additional relationship marks follow **GenoPro**. Emotional-relationship taxonomy follows **GenoPro's 32-type classification** (we do not implement GenoPro's 34-type set in full; `focused-admirer` and one duplicate are folded into `focused` and `admirer`).
...
The title and explicit person labels are editable. Individuals move horizontally within a generation; generation depth remains semantic while structural and emotional relationship lines follow live.
