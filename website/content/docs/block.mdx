...
|---|---|---|
| `role: plant` | `plant`, `controller`, `sensor`, `actuator`, `reference`, `disturbance`, `input`, `output`, `generic` | Visual color coding; `generic` is the default |
...
| Unknown `role:` value | Renders with neutral `generic` styling and a structured `blockdiagram/unknown-role` warning | Use a first-class role when one is semantically accurate |
| Unknown connection attribute | Ignored with a structured `blockdiagram/unknown-connection-attribute` warning | Check the supported label and `discrete` attributes |
