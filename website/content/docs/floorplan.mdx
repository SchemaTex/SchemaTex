...
`start` means west on a horizontal wall and north on a vertical wall. `from end n` measures the leading jamb back from the opposite end. A `before` / `after` reference must name an earlier opening on the same resolved wall, so source order stays readable and deterministic.
