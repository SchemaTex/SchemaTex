...
## 1. A minimal circuit (netlist mode — recommended)
...
Fixed symbols accept `pins="..."` using their real anchor names. The nodes bind in that exact order; no coordinates or new component type are needed. An op-amp defaults to `plus,minus,out`. To include power connections:

```text
U1 input feedback output VCC VEE type=opamp pins="plus,minus,out,supply+,supply-" label="LM741"
R1 output feedback 10k
R2 feedback 0 2.2k
```

Use `value=` for a component value when declaring pins explicitly. Unknown or duplicate pin names are rejected; surplus op-amp/comparator nets produce `CIRCUIT_PIN_OVERSPECIFIED` instead of becoming display text. A floating feedback net remains a diagnostic: the renderer never invents the missing connection. Named supply flags indicate shared nets; this is not electrical simulation or a manufacturer-specific pin-number model.
...
| `solar_cell` | IEC photovoltaic cell + incident-light arrows (also: `solar`) |
...
| `triode` | IEEE 315 thermionic valve with plate/grid/cathode pins |
...
| `generic_ic` | Configurable rect with labeled pins (also: `ic`, `mcu`, `gear_ctrl`) |
| `voltage_regulator` | 3-terminal block: IN/GND/OUT (also: `reg`, `regulator`, `ldo_3v3`) |
...
| DSL type | Description |
|---|---|
| `switch_spst` | Single-pole normally-open maintained switch (also: `switch_no`, `switch_spst_no`) |
| `switch_nc` | Maintained SPST normally-closed contact (also: `switch_spst_nc`) |
...
| `relay` | Combined coil + normally-open contact (4 pins) |
| `relay_spdt` | Combined coil + changeover contact (5 pins) |
...
|---|---|
| `motor` | Circle + M |
| `fan` | IEC rotating-machine fan / impeller |
| `lamp` | Lighting load: circle + X (aliases: `light`, `bulb`) |
| `speaker` | Cone + box |
...
Common production aliases use the closest canonical symbol without replacing a user's `label=`: `pushbutton`→`push_no`, `ntc`→`thermistor_ntc`, `dc_supply`→`voltage_source`, `selector`→`selector_switch`, `pullup`→`resistor`, `dc_motor`→`motor`, `outlet`→`mains_socket`, and `prox_sensor`→`proximity_sensor`.

An explicit unknown `type=` is not fatal. Schematex draws a neutral rectangle labeled with the raw type (or authored label), binds every declared net through the same numbered-pin path as `generic_ic`, and returns a structured `circuit/unknown-component-type` warning containing `token` and `line`. Missing/invalid statements, malformed netlists, and ids with neither an inferable prefix nor an explicit type still fail validation.
...
### 4.5 Household lighting circuits

Use explicit `type=lamp` for a lighting load because a bare `L` prefix means **inductor** in SPICE. Keep the source on named `live` / `neutral` nets; auto-layout places the protected live path above a lower neutral return rail.
...
For two-location (stair) switching, use two SPDT switches sharing two traveler nets. Series SPST switches describe a different circuit and cannot provide normal two-way control.
...
Use `L1` (or another load-appropriate designator) for a lamp. Reserve `RL1` for an actual relay or load resistor.
...
The title and explicit component label/value attributes are editable. Stable component IDs move freely. In netlist mode every attached net previews from the moving component and is regenerated as horizontal/vertical orthogonal segments after the drop; generated wire identities remain protected.
