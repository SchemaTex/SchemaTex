...
For a complete 555 astable, keep using ordinary netlist syntax. The renderer
recognizes the functional topology from the 555 pin nets—component IDs are not
special—and separates the timing network from the output branch automatically.

<Playground initial={`circuit "555 Astable LED Flasher" netlist
...
D1 LED_A GND type=led label="LED1"`} height={520} />
