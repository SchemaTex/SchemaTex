...
| `generic_ic` | Configurable rect with labeled pins (also: `ic`, `mcu`) |
...
Common production aliases use the closest canonical symbol without replacing a user's `label=`: `pushbutton`→`push_no`, `ntc`→`thermistor_ntc`, `dc_supply`→`voltage_source`, `selector`→`selector_switch`, `pullup`→`resistor`, and `dc_motor`→`motor`.
