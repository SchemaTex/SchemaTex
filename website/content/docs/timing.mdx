...
EN:   01111110
```
Verify: CLK is a square wave, DATA has 8 trapezoid segments with text labels, EN has correct 0/1 transitions.
...
```
Verify: `z` renders as a dashed midline, `x` renders as cross-hatch fill.
...
```
Verify: each group of signals has a bracket, CLK_B phase offset is 0.5 periods.
...
```
Verify: red bidirectional arrows + labels are correctly positioned near the CLK rising edge.
...
RST: 10000001
```
Verify: single signal does not crash, canvas size scales correctly.

### Case 6: hscale Configuration
```
timing [hscale: 3]
A: 01010101
B: 00110011
```
Verify: period_width = 40 × 3 = 120px, each period is noticeably wider.
...
|----------|---------|------------|---------|
| P0 (MVP) | `0/1/p/n/.` basic states + sharp transitions | Low | Core — used in every timing diagram |
| P0 | Signal name column + horizontal layout | Low | Core |
| P0 | `=` data values + data[] label array | Low | High — core of bus signals |
| P1 | `x` unknown (cross-hatch) + `z` hi-Z (dashed) | Low | High — required for digital design |
| P1 | Clock waveforms (p/n/P/N) | Low | High |
| P1 | Group notation `[name]` + bracket | Medium | High |
| P1 | `hscale` config | Low | Medium |
| P2 | Timing annotations (arrows, labels) | Medium | Medium — professional use |
