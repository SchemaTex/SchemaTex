...
其余 30 种图仍可正常 render，也能通过 DSL 源码编辑，但会返回空 `text` 与 `position: "none"`，并且不会生成猜测式 Canvas handle。
...
[Interactive Workspace](/playground) 包含全部 51 种图的已发布案例。能力栏直接由同一份 registry 生成，因此 parser-native Canvas 编辑与 source-only 行为不会发生文案漂移。
