...
Branches, shape labels, edge labels, and the title are exact-range text targets. Stable nodes move freely on both axes; connected edges preview during the gesture and reroute from their semantic endpoints after the drop. Both top-down and left-to-right layouts preserve these guarantees.
