...
**Skip this if your app already has a rendering pipeline.** If you're building on top of the `schematex` package (Path 1), call `renderDsl` from the library directly — don't round-trip through the tool layer. The tool version is intended for pure chat clients (Claude Desktop, ChatGPT) that need the SVG returned inline in the conversation.
...
[**→ Open "Add custom connector" in Claude.ai**](https://claude.ai/settings/connectors?modal=add-custom-connector)

Clicking the link above opens the connector dialog directly. Paste `https://schematex.js.org/mcp` as the server URL and click **Add**. The connector will appear under Settings → Connectors and is available in every conversation.

> Requires a Claude.ai account (Free / Pro / Team / Enterprise).
