...
### `listDiagrams()`
Returns all 45 diagram types with tagline, "use when" hint, cluster, and authoritative standard. Call first to pick a type.
...
> **MCP vs npm:** `buildPromptContext` is only available in the `schematex/ai` npm package (Path 1 below). The hosted MCP server (Path 2) exposes `getSyntax` and `getExamples` as separate tool calls — the agentic loop does the composing for you.
