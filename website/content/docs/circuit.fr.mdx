...
|---|---|
| `motor` | Cercle + M |
| `lamp` | Charge d’éclairage : cercle + X (alias : `light`, `bulb`) |
...
### 4.5 Éclairage domestique

Utilisez explicitement `type=lamp` pour une lampe, car un préfixe `L` nu désigne une **inductance** en SPICE. Avec les réseaux `live` et `neutral`, l’auto-layout place la phase protégée en haut et le neutre sur un retour inférieur.
...
Pour une commande depuis deux emplacements, utilisez deux commutateurs SPDT partageant deux navettes. Des SPST en série décrivent un autre circuit.
...
Utilisez `L1` pour la lampe ; réservez `RL1` à un véritable relais ou une résistance de charge.
