...
Basketball and soccer actions run in declaration order. A cut, dribble or run updates the player's position for later passes; a pass or shot does not move the player. Player symbols show starting positions, and numbered actions show the sequence. Football assignments remain simultaneous from the snap formation.
