...
|---|---|---|
| Rectangular | `rectangular` | L-shaped branches; root on left, tips on right |
| Slanted | `slanted` | Default. Diagonal lines from parent to child; more compact |
...
**Slanted (default)** — diagonal lines join parent and child directly. Layout is separate from branch-length semantics: in `phylogram`, horizontal displacement represents distance, not the diagonal line length. Choose `mode: cladogram` for a topology-only triangular tree with aligned tips; choose `layout: rectangular` for elbow branches. Clustering dendrograms retain rectangular connectors.
...
|---|---|---|---|
| `layout:` | `rectangular`, `slanted`, `circular`, `unrooted` | `slanted` | Tree layout |
