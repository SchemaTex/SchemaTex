...
| `RES` | Reset | Rockwell / Allen-Bradley counter or timer reset instruction, drawn inline as `[RES]` |

**Syntax:** identical to contacts — `OTE(tag)`, `OTE(tag, "address")`, `OTE(tag, "address", name="…")`.

`OTL` and `OTU` draw Allen-Bradley `(L)` and `(U)` coils. `ONS` draws the bracketed inline instruction `[ONS]`.
...
Timer and counter blocks have a ruled header separating the instruction name from its parameters. Function blocks perform timer, counter, math, and comparison operations. They appear inline in a rung and have keyword parameters after the mandatory tag argument.
...
**Element names** are all uppercase ASCII: `XIC`, `XIO`, `ONS`, `OSF`, `OTE`, `OTL`, `OTU`, `OTN`, `RES`, `TON`, `TOFF`, `TP`, `CTU`, `CTD`, `CTUD`, `ADD`, `SUB`, `MUL`, `DIV`, `MOV`, `EQU`, `NEQ`, `GRT`, `LES`, `GEQ`, `LEQ`.
...
- ✅ Four coil drawings: OTE (output), OTL (L, latch), OTU (U, unlatch), OTN (negate), plus the bracketed RES reset instruction
