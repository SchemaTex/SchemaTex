...
Machine client 无需抓取渲染后的网页即可发现完整公共接口：[`/llms.txt`](/llms.txt) 是精选索引，
[`/llms-full.txt`](/llms-full.txt) 包含完整英文文档，每个 docs URL 都有 `.md` mirror，
[`/api/interactive-capabilities`](/api/interactive-capabilities) 则以 JSON 输出 canonical editor capability registry。
...
### `listDiagrams()`
返回全部 51 种图表类型，包含简介、使用场景提示、所属 cluster、权威标准和 Interactive 能力摘要。首先调用此工具来选择类型。
