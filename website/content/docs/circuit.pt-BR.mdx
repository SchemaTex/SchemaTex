...
|---|---|
| `motor` | Círculo + M |
| `lamp` | Carga de iluminação: círculo + X (aliases: `light`, `bulb`) |
| `speaker` | Cone + caixa |
...
### 4.5 Iluminação residencial

Use `type=lamp` explicitamente para uma lâmpada, pois um prefixo `L` sem tipo significa **indutor** em SPICE. Com redes `live` e `neutral`, o auto-layout coloca a fase protegida acima e o neutro como retorno inferior.
...
Para controle em dois locais, use dois interruptores SPDT compartilhando duas redes viajantes. Interruptores SPST em série descrevem outro circuito.
...
Use `L1` para a lâmpada; reserve `RL1` para um relé ou resistor de carga real.
