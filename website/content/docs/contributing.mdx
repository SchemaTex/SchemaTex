---
title: Contributing a new diagram type
description: Step-by-step guide for adding a new diagram plugin to Schematex — from standard spec to published website example.
---


> A step-by-step guide for adding a new diagram plugin to Schematex — from standard spec to published website example. Read [`00-OVERVIEW.md`](/docs) first for the overall architecture.
...
- **Renderer** — string-building SVG via [`src/core/svg.ts`](https://github.com/victorzhrn/Schematex/blob/main/src/core/svg.ts); no DOM, SSR-safe.
...
2. **Strict TypeScript.** No `any`, no un-commented `as`. Types in [`src/core/types.ts`](https://github.com/victorzhrn/Schematex/blob/main/src/core/types.ts) are the spec.
...
4. **Use the SVG builder.** Never concatenate raw SVG strings — use [`src/core/svg.ts`](https://github.com/victorzhrn/Schematex/blob/main/src/core/svg.ts).
...
Look at [`06-TIMING-STANDARD.md`](/docs/timing) or [`11-SINGLE-LINE-STANDARD.md`](/docs/sld) as templates.
...
Edit [`src/core/api.ts`](https://github.com/victorzhrn/Schematex/blob/main/src/core/api.ts):
...
1. **Gallery tile** — add an entry to [`website/lib/gallery-data.ts`](https://github.com/victorzhrn/Schematex/blob/main/website/lib/gallery-data.ts). The `dsl` field must parse — validate with `node scripts/validate-gallery.mjs`.
2. **Static SVG** — add an entry to [`scripts/generate-gallery-svgs.mjs`](https://github.com/victorzhrn/Schematex/blob/main/scripts/generate-gallery-svgs.mjs) and run it. The generated SVG ships with the repo and is referenced from the README.
...
- [`README.md`](https://github.com/victorzhrn/Schematex/blob/main/README.md) — gallery row.
- [`CLAUDE.md`](https://github.com/victorzhrn/Schematex/blob/main/CLAUDE.md) — "Completed" list at the bottom.
- [`docs/reference/00-OVERVIEW.md`](/docs) — status table.
...
- **Inline `style=` attributes** — blocked by the semantic-SVG rule. Use CSS classes and expose them as theme tokens in [`src/core/theme.ts`](https://github.com/victorzhrn/Schematex/blob/main/src/core/theme.ts).
...
|-----------|--------|-----------|
| Minimal | [`timing`](https://github.com/victorzhrn/Schematex/tree/main/src/diagrams/timing) | Parser + renderer only, no separate layout. |
| Medium | [`ecomap`](https://github.com/victorzhrn/Schematex/tree/main/src/diagrams/ecomap) | Clean AST → layout → renderer split. |
| Advanced | [`genogram`](https://github.com/victorzhrn/Schematex/tree/main/src/diagrams/genogram) | Generational layout, multi-pass routing, rich symbol set. |
| Advanced | [`sld`](https://github.com/victorzhrn/Schematex/tree/main/src/diagrams/sld) | Voltage-level banding, bus routing, device clustering. |
