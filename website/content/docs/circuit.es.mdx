...
|---|---|
| `motor` | Círculo + M |
| `lamp` | Carga de iluminación: círculo + X (alias: `light`, `bulb`) |
| `speaker` | Cono + caja |
...
### 4.5 Iluminación doméstica

Use `type=lamp` de forma explícita para una lámpara, ya que un prefijo `L` sin tipo significa **inductor** en SPICE. Con redes `live` y `neutral`, el auto-layout coloca la fase protegida arriba y el neutro como retorno inferior.
...
Para control desde dos ubicaciones, use dos interruptores SPDT que compartan dos redes viajeras. Varios SPST en serie describen un circuito distinto.
...
Use `L1` para la lámpara; reserve `RL1` para un relé o una resistencia de carga reales.
