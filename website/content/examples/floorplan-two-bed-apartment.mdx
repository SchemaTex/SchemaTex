...
description: "A fully furnished residential plan with separate exterior/interior wall weights, a measured two-window façade chain, shared-wall doors, automatic labels, computed room areas, and exterior dimensions."
diagram: floorplan
...
dsl: |
  ...
  window living north from start 0.4 width 1.8 id living-west
  window living north after living-west gap 1.0 width 1.2 id living-east
...
The whole apartment is placed with `right-of` / `below` chaining from one `at 0,0` anchor — adjacent rooms share an edge exactly, so their walls merge into a single poché band. `wall exterior` and `wall interior` establish a visible structural hierarchy while every interior door still uses `door between A B`, which finds the shared wall automatically.

The living-room façade is measured rather than percentage-guessed: `living-west` starts 0.4 m from the west jamb and `living-east` follows it with an exact 1.0 m gap. Opening ids are source-order identities on one resolved wall, so changing the first width moves the second predictably.

Room areas (and the 68 m² total in the SVG `<desc>`) are computed from the geometry, never typed. Labels automatically avoid furniture envelopes and door swings. The kitchen shows the underlay convention: the sink and stove sit *inside* the counter run without triggering collision warnings, while a misplaced wardrobe would be flagged with the overlap in meters.
