...
3. He was **removed due to physical abuse** from both bio parents — zigzags with filled arrowheads in the theme’s `negative` ink.
4. The **maternal uncle** is Mónica's brother (`sibling-of: monica`) with **no current relationship** to Isaías — dashed bracket between Mónica and Tío + `nevermet` line with a boxed X at its midpoint.
...
- `-physical-abuse->` → zigzag with a filled arrowhead in the theme’s `negative` ink; the `>` indicates the perpetrator (left side) and victim (right side).
