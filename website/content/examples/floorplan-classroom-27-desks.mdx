...
`grid desk-chair … count 27` places a 5×6 grid and drops the last row's tail — exactly how a real 27-student room fills. The `area` corners are item centers, so respacing the whole room is a two-number edit.
