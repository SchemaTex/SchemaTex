...
interface GithubStarButtonProps {
  stars: number;
  size?: 'sm' | 'md';
  /** Plausible event suffix so we can tell which placement converts (header/hero/cta/…). */
  source?: string;
}

export function GithubStarButton({ stars, size = 'md', source = 'unknown' }: GithubStarButtonProps) {
  ...
  return (
    <a
      ...
      onClick={() => {
        plausible('star_click', { props: { source } });
        setClicked(true);
      }}
    ...
    >
      ...
      >
        <span
          ...
          className={[
            'star-glyph',
            clicked ? 'star-pop' : size === 'md' ? 'star-twinkle-idle' : '',
          ]
            .filter(Boolean)
            .join(' ')}
        >
