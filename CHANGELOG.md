...
- **74-symbol auto-seating furniture catalog** across the four demand clusters (residential incl. sectional/fireplace/grand piano/ceiling fan; kitchen/bath incl. dashed wall-cabinet & range-hood above-cut-plane conventions, double vanity, urinal; classroom/office incl. desk-l, lockers, filing; event/banquet) — `round-table-8` *is* 8 countable chairs (60″ top), dining/banquet/conference tables seat both long edges at 0.65 m pitch, `row-chairs` places theater strips at 0.55 m. Arrays: `grid`/`row` with row-major `count` truncation (27 desks in a 5×6 grid drops the tail), `arc` for semicircle seating facing center.
...
- **Surfaces**: `/icons` ships the full 74-symbol sheet rendered by the real engine; 9 worked examples across the demand clusters (apartment, 27-desk classroom, 120-guest wedding, 160 m² family home, bistro café, kindergarten, studio office, 96-seat lecture hall, semicircle garden ceremony); docs page with playground; generation profile + registry entry under the new **Architecture & Space** cluster.
